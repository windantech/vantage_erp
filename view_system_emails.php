            ...
            <div class="card shadow mb-4 rounded-0">
                <div class="card-header bg_main rounded-0 py-3">
                    <div class="w-100 d-flex align-items-center">
                        <div class="w-50">
                            <h6 class="m-0 font-weight-bold text-white text-uppercase">View System Email</h6>
                        </div>
                ...
                <div class="card-body">
                    ...
                    $query = "SELECT * FROM system_emails WHERE id = '$id'";
                    $result = $conn->query($query) or die($conn->error);

                    if ($result->num_rows > 0) {
                        $sm__data = $result->fetch_assoc();

                        // Extract the title and subject as usual
                        $email_title = $sm__data['title']; // Assuming you want the title field
                        $email_subject = $sm__data['subject']; // Assuming subject field exists

                        // Decode the JSON body field into an associative array
                        $body = json_decode($sm__data['body'], true);

                        // Use the keys of the array to assign values to variables
                        $alert_title = $body['alert_title'] ?? ''; // First part of the body
                        $main_content = $body['main_content'] ?? ''; // Second part of the body
                        $call_to_action = $body['call_to_action'] ?? ''; // Third part of the body
                        $invitation_message = $body['invitation_message'] ?? ''; // Fourth part of the body
                        $schedule_link = $body['schedule_link'] ?? ''; // Fifth part of the body
                        $trainer_link = $body['trainer_link'] ?? ''; // Sixth part of the body
                        $training_details = $body['training_details'] ?? ''; // Seventh part of the body
                        $elearning_link = $body['elearning_link'] ?? ''; // Eighth part of the body
                        $training_reasons = $body['training_reasons'] ?? ''; // Ninth part of the body
                        $cta_link = $body['video_link'] ?? ''; // Tenth part of the body
                        $whatsapp_link = $body['whatsapp_link'] ?? ''; // Eleventh part of the body
                        $registration_link = $body['registration_link'] ?? ''; // Twelfth part of the body
                        $investment_text = $body['investment_text'] ?? ''; // Thirteenth part of the body
                        $coordinator_link = $body['class_coordinator_link'] ?? ''; // Fourteenth part of the body
                        $referral_link = $body['referral_link'] ?? ''; // Fifteenth part of the body
                    ...
                    <div style="border: solid 1px #d1d3e2;">
                        <div style="padding: 0 .5rem">
                            ...
                            <div style="width: 100%; overflow: hidden;">
                                <a href="<?= htmlspecialchars($cta_link) ?>" target="_blank" id="cta_preview" style="float: left; width: 40%; padding: 10px; text-align: center; text-decoration: none; color: black;">
                                ...
                                <a id="cta_custom_preview" href="<?= htmlspecialchars($cta_link) ?>" target="_blank" style="float: left; width: 40%; padding: 10px; text-align: center; text-decoration: none; color: black;">
                            ...
                            <div style="width: 100%; overflow: hidden;">
                                ...
                                <a id="coordinator_link" href="<?= htmlspecialchars($coordinator_link) ?>" target="_blank" style="float: left; width: 24%; padding: 2px; text-align: center; text-decoration: none; color: black;">
