...
function count_entries($conn, $id, $date_from = null, $date_to = null) {
    // Base query
   $query = "SELECT DISTINCT(email) FROM `register` WHERE `program` = '$id' AND status=2 ";
...
if (mysqli_num_rows($check_course)) {
    while ($row = mysqli_fetch_array($check_course)) {
        $dataPoints[] = [
            ...
            "y" => (int)(count_entries($conn, $row["course_id"], $date_from, $date_to)/count_entries_($conn, $row["course_id"], $date_from, $date_to) * 100),  // Pass date filters to count_entries
