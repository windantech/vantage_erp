...
function count_entries($conn, $id, $date_from = null, $date_to = null) {
   ...
   $query= "SELECT event_id,`id`, `fullname`, `email`, `term`, `phone_number`, `ticket_id`, `status`, `amount`, `ticket_number`, `confirmation`, `date_sent` FROM `ticket_congress`  WHERE `event_id` = '$id' AND status=2 ";
...
function count_entries_($conn, $id, $date_from = null, $date_to = null) {
    // Base query
   $query = "SELECT DISTINCT(email) FROM `ticket_congress` WHERE `event_id` = '$id'  ";
...
if ($hyphenPosition !== false) {
    ...
    $firstThreeLetters = substr($afterHyphen, 0, 3);
      $denominator = count_entries_($conn, $row["event_id"], $date_from, $date_to);

        $dataPoints[] = [
            "label" => strtoupper($firstThreeLetters),  // Get the 'label' column
     
      "y" => $denominator > 0 ? (int)(count_entries($conn, $row["event_id"], $date_from, $date_to) / $denominator * 100) : 0,
