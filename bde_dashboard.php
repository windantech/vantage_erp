      ...
      function vStrategy(){
        ...
        return `
          ...
          <section class="grid-2">
            ...
            <div class="card"><div class="chead"><h4>Performance scorecard</h4><span class="chip jade">Live · your actuals</span></div><div class="scard">${score.map(([n,w,val,frac])=>`<div class="scrow"><div class="st"><label>${esc(n)}</label><span class="w">${w}% weight</span></div><div class="sv">${frac==null?"":`<b>${pct(frac)}</b> · `}${esc(val)}</div>${frac==null?"":`<div class="sb"><i style="width:${Math.round(clamp(frac)*100)}%"></i></div>`}</div>`).join("")}</div></div>
          </section>
          <section class="grid-3">
