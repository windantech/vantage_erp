...
// Department/mandate fallback: Digital Solutions (and other) BDEs often have NO resolvable department
// (no CRM assignment, blank department_id) so bde_mandate() lands on the generic text. Their real
...
if ($bde_metrics) {
    ...
    if (!$known && $blob !== '') {
        ...
        elseif (strpos($blob, 'international') !== false) { $inferred = 'International'; }
        elseif (preg_match('/course|virtual/', $blob)) { $inferred = 'Virtual'; }
        if ($inferred !== '') {
...
if ($bde_is_admin) {
    ...
    // Exclude HOD/BDO accounts (anyone carrying a department-total target). BDOs live on
    // bdo_dashboard.php — the BDE dashboard stays BDE-only.
    $bdoIds = [];
    ...
    if (!empty($bdoIds)) { $bde_people = array_values(array_filter($bde_people, function ($p) use ($bdoIds) { return !isset($bdoIds[$p['id']]); })); }
...
<?php endif; ?>
      ...
      function kpiBlock(){
        ...
        const items=[
          ...
          ["Remaining to target",hasRev?kMoney(Math.max(0,B.target-B.actual)):"—",hasRev?"target − collected":"count-based target","flat","var(--slate)"],
      ...
      function vPipeline(){
        ...
        // Enough real activity to judge quality? Otherwise "healthy" is misleading (nothing to assess).
        const hasActivity=(leadN>5)||((B.actual||0)>0)||((B.paidClients||0)>0);
