      ...
      function vReport(){
        ...
        return `
          ...
          <section class="grid-3">
            ${[['<svg viewBox="0 0 24 24"><path d="M4 20V10M10 20V4M16 20v-6M22 20H2"/></svg>',"Auto-filled from your data","Consolidated revenue, SBU attainment, pipeline, collections, International clients and personal sales are pulled live across the SBUs — check and adjust if needed."],['<svg viewBox="0 0 24 24"><path d="M12 3l2.4 5 5.6.5-4.2 3.7 1.3 5.5L12 20l-5.1 2.7 1.3-5.5L4 8.5 9.6 8z"/></svg>',"Your judgement","Explain why the weak SBUs moved, which strategic accounts are blocked, what was decided and which CEO support you need."],['<svg viewBox="0 0 24 24"><path d="M9 12l2 2 4-4"/><path d="M21 12a9 9 0 1 1-9-9"/></svg>',"CEO review","The CEO reviews, comments, approves or returns it, and turns your commitments into tracked actions."]].map(([ic,a,b])=>`<div class="card"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><span style="width:36px;height:36px;flex:none;border-radius:9px;background:var(--surface2);display:grid;place-items:center;color:var(--brand)"><span style="display:grid;place-items:center;width:19px;height:19px">${ic.replace('<svg','<svg style="width:19px;height:19px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round"')}</span></span><h4 style="margin:0">${esc(a)}</h4></div><p style="font-size:12.5px;color:var(--muted);margin:0;line-height:1.5">${esc(b)}</p></div>`).join("")}
          </section>`;
      }
