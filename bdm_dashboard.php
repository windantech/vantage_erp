  ...
  <div id="content">
    ...
    (() => {
      ...
      function execRevenueBreakdown(){
        ...
        const max=(Math.max(1,...data.flatMap(d=>[d.closed,d.open])))*1.18;
        const w=660,h=260,pd=36,base=h-pd-16,plot=base-pd,step=(w-2*pd)/Math.max(1,data.length),bw=24,gap=8;
        const bars=data.map((d,i)=>{const cx=pd+step*i+step/2;const xC=cx-bw-gap/2,xO=cx+gap/2;const ch=d.closed/max*plot,oh=d.open/max*plot;return `<g>
          <rect x="${xC.toFixed(1)}" y="${(base-ch).toFixed(1)}" width="${bw}" height="${Math.max(0,ch).toFixed(1)}" rx="3" fill="var(--jade)"/>
          <rect x="${xO.toFixed(1)}" y="${(base-oh).toFixed(1)}" width="${bw}" height="${Math.max(0,oh).toFixed(1)}" rx="3" fill="#4d8bd6"/>
          <text x="${(xC+bw/2).toFixed(1)}" y="${(base-ch-5).toFixed(1)}" text-anchor="middle" style="font-size:9px;font-weight:800;fill:var(--jade)">${d.closed>0?kMoney(d.closed):""}</text>
          <text x="${(xO+bw/2).toFixed(1)}" y="${(base-oh-5).toFixed(1)}" text-anchor="middle" style="font-size:9px;font-weight:800;fill:#4d8bd6">${d.open>0?kMoney(d.open):""}</text>
          <text x="${cx.toFixed(1)}" y="${base+16}" text-anchor="middle">${esc(d.name)}</text></g>`;}).join("");
        return `<div class="card"><div class="chead"><div><h4>Cleared revenue vs open pipeline — by SBU</h4><p>Cleared = collected &amp; Finance-verified. Open pipeline = expected from current registrations, not yet collected (unweighted, so it can exceed target).</p></div></div>
          <svg class="chart" viewBox="0 0 ${w} ${h}" style="height:260px" role="img" aria-label="Cleared revenue vs open pipeline by SBU">${[0,.25,.5,.75,1].map(t=>`<line class="grid" x1="${pd}" y1="${(pd+t*plot).toFixed(1)}" x2="${w-pd}" y2="${(pd+t*plot).toFixed(1)}"/>`).join("")}${bars}</svg>
          <div class="legend"><span class="lg"><i style="background:var(--jade)"></i>Cleared (collected)</span><span class="lg"><i style="background:#4d8bd6"></i>Open pipeline (expected)</span></div></div>`;
      }
