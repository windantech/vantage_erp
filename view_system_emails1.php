...
<section id="content-wrapper" class="d-flex flex-column">
    <div id="content">
        ...
        if ($result->num_rows > 0) {
            ...
            // Get email type (default to 'virtual' for backward compatibility)
            $email_type = isset($sm__data['email_type']) ? $sm__data['email_type'] : 'virtual';
            $email_type_label = ($email_type === 'international') ? 'International Event' : 'Virtual Course';
            $email_type_badge = ($email_type === 'international') 
                ? '<span class="badge bg-danger">International</span>' 
                : '<span class="badge bg-primary">Virtual</span>';
            
            // Get course or event name based on type
            if ($email_type === 'international') {
                $target_name = isset($sm__data['event_name']) && !empty($sm__data['event_name']) 
                    ? $sm__data['event_name'] 
                    : 'N/A';
            } else {
                $target_name = !empty($sm__data['course_opt']) ? $sm__data['course_opt'] : 'N/A';
            }
            
            // Build view header
            $s_view = (!empty($target_name) && !empty($sm__data['email_opt']))
                ? "(" . $target_name . " - Email " . $sm__data['email_opt'] . ")"
                : "";
            ...
            <div class="card shadow mb-4 rounded-0">
                <div class="card-header bg_main rounded-0 py-3">
                    <div class="w-100 d-flex align-items-center">
                        <div class="w-50">
                            <h6 class="m-0 font-weight-bold text-white text-uppercase">View System Email <?= $s_view ?></h6>
                        </div>
                        <div class="w-50 d-flex justify-content-end align-items-center">
                            <button onclick="location.href='edit_system_emails1?id=<?= $id ?>'" class="btn border-0 p-0 ms-3">
                            ...
                            <div class="card rounded-0">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 mb-2">
                                            <strong><?= ($email_type === 'international') ? 'Event' : 'Course' ?>:</strong><br>
                                            <?= htmlspecialchars($target_name) ?>
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <strong>Email Number:</strong><br>
                                            Email <?= htmlspecialchars($sm__data['email_opt']) ?>
                                        </div>
                                        <div class="col-md-3 mb-2">
                                            <strong>Subject:</strong><br>
                                            <?= htmlspecialchars($sm__data['subject']) ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
