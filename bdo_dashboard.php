...
$bdo_series = function_exists('bde_daily_series') ? bde_daily_series(($bdo['daily'] ?? []), $bdo_to) : ['cum' => [], 'amt' => [], 'dates' => [], 'dim' => 30, 'dom' => 1];
...
<?php if ($bdo): ?>
      ...
      Object.assign(B, {
        ...
        clearedKes: <?php echo (float) ($bdo['clearedKes'] ?? 0); ?>,
      ...
      B.isSeller = <?php echo !empty($bdo['bdoIsSeller']) ? 'true' : 'false'; ?>;
      B.dailyCum = <?php echo json_encode($bdo_series['cum'], JSON_INVALID_UTF8_SUBSTITUTE) ?: '[]'; ?>;
      B.dailyAmt = <?php echo json_encode($bdo_series['amt'], JSON_INVALID_UTF8_SUBSTITUTE) ?: '[]'; ?>;
      B.dailyDates = <?php echo json_encode($bdo_series['dates'], JSON_INVALID_UTF8_SUBSTITUTE) ?: '[]'; ?>;
      B.daysInMonth = <?php echo (int) $bdo_series['dim']; ?>; B.dayToday = <?php echo (int) $bdo_series['dom']; ?>;
...
<?php endif; ?>
      ...
      // Department revenue chart: real month-to-date daily cleared KES for revenue depts; the synthetic
      // pace line for International (client-based) or when there's no daily data.
      function deptTrendSVG(){
        if(isCount||!(B.dailyCum&&B.dailyCum.length)) return trendSVG();
        const series=B.dailyCum;const dim=Math.max(2,B.daysInMonth||30);const dayT=Math.max(1,Math.min(dim,B.dayToday||series.length));
      ...
      function teamTable(){
        ...
        return `<div class="card tight"><div class="table-wrap"><table><thead><tr><th>Employee</th><th>Cleared revenue</th><th>Paid clients</th><th>vs Target</th><th>Status</th>${B.canNote?'<th style="width:96px">Note</th>':""}</tr></thead><tbody>${B.team.map((t,i)=>{const ini=t.name.split(/\s+/).map(x=>x[0]).slice(0,2).join("");const isMgr=!!t.manager;const p=period();let clearedCell,clientsCell,vs,statusCell;if(isMgr){/* BDO who oversees: their row summarises the WHOLE department they own */const dTgt=(+B.target)||0,dAct=(+B.actual)||0,dCli=(+B.clients)||0,dKes=(+B.clearedKes)||0;const dAtt=dTgt?dAct/dTgt:null;const dstt=dAtt==null?"":dAtt>=1?"green":dAtt>=0.7?"amber":"red";const dexp=dTgt*(p.elapsed/p.working);const dst=dTgt<=0?"none":dAct>=dexp?"green":dAct>=dexp*.85?"amber":"red";const dlbl=dst==="green"?"On pace":dst==="amber"?"At risk":"Behind pace";clearedCell=`${kMoney(dKes)}<div style="font-size:9.5px;color:var(--muted);font-weight:600;margin-top:2px">whole department</div>`;clientsCell=`${nf.format(dCli)}`;vs=dAtt==null?'<span style="color:var(--muted)">—</span>':`<span class="mini-track"><div style="width:${clamp(dAtt*100,0,100)}%;background:${scol(dstt)}"></div></span> <b class="num" style="font-size:11.5px;color:${scol(dstt)}">${pct(dAtt,0)}</b><div style="font-size:10px;color:var(--muted)">of ${mfmt(dTgt)} · dept total</div>`;statusCell=dst==="none"?'<span class="chip slate">Team lead</span>':`<span class="sbadge s${dst[0]}"><span class="dot"></span>${dlbl}</span>`;}else{const tgt=isCount?(+t.clientsTarget||0):(+t.target||0);const act=isCount?(+t.clients||0):(+t.actual||0);const hasT=tgt>0;const att=hasT?act/tgt:null;const stt=att==null?"":att>=1?"green":att>=0.7?"amber":"red";const exp=hasT?tgt*(p.elapsed/p.working):0;const st=!hasT?"none":act>=exp?"green":act>=exp*.85?"amber":"red";const lbl=st==="green"?"On pace":st==="amber"?"At risk":"Behind pace";const noneLbl=isCount?"No client target":"No revenue target";vs=hasT?`<span class="mini-track"><div style="width:${clamp(att*100,0,100)}%;background:${scol(stt)}"></div></span> <b class="num" style="font-size:11.5px;color:${scol(stt)}">${pct(att,0)}</b><div style="font-size:10px;color:var(--muted)">of ${isCount?nf.format(tgt)+" clients":kMoney(tgt)}</div>`:'<span style="color:var(--muted)">no target set</span>';clearedCell=`${kMoney(t.actual||0)}`;clientsCell=`${nf.format(t.clients||0)}`;statusCell=st==="none"?`<span class="chip slate">${noneLbl}</span>`:`<span class="sbadge s${st[0]}"><span class="dot"></span>${lbl}</span>`;}return `<tr class="${t.me?"me":""}"><td><div class="prow"><span class="a"${t.me?"":` style="background:${avatarCols[i%avatarCols.length]}"`}>${ini}</span><div><b>${esc(t.name)}${t.me?" · you":""}</b><span>${esc(t.title)}${isMgr?" · whole dept":""}</span></div></div></td><td class="num">${clearedCell}</td><td class="num">${clientsCell}</td><td>${vs}</td><td>${statusCell}</td>${B.canNote?`<td>${t.id?`<button type="button" class="notebtn tbtn" data-bde="${t.id}" data-name="${esc(t.name)}" style="padding:5px 11px;font-size:11.5px;white-space:nowrap">✎ ${(B.notes&&B.notes[t.id]&&((((B.notes[t.id].t||"").trim())||((B.notes[t.id].c||"").trim())))?"Edit":"Add note")}</button>`:""}</td>`:""}</tr>`;}).join("")}</tbody></table></div></div>`;
      }
      ...
      function bdoPersonalSection(){
        // Only genuine individual sellers (e.g. Edwin) get a personal block. A manager BDO
        // co-tagged on the team's programmes (e.g. Erick) has no distinct personal sales.
        if(!B.isSeller) return "";
      ...
      function vCommand(){
        ...
        return `${strategyStrip()}
          ...
          <section class="grid-2">
            <div class="card"><div class="chead"><h4>Revenue pace &amp; month-end forecast</h4><span class="chip jade">${mfmt(B.forecast)} forecast</span></div>${deptTrendSVG()}<div style="font-size:11.5px;color:var(--muted);margin-top:10px">The forecast moves whenever stage, probability, payment date or cleared revenue changes.</div></div>
            ${commissionMini()}
          </section>
