...
<?php endif; ?>
      ...
      function teamTable(){
        ...
        return `<div class="card tight"><div class="table-wrap"><table><thead><tr><th>Employee</th><th>Cleared revenue</th><th>Paid clients</th><th>vs Target</th><th>Status</th>${B.canNote?'<th style="width:96px">Note</th>':""}</tr></thead><tbody>${B.team.map((t,i)=>{const ini=t.name.split(/\s+/).map(x=>x[0]).slice(0,2).join("");const tgt=isCount?(+t.clientsTarget||0):(+t.target||0);const act=isCount?(+t.clients||0):(+t.actual||0);const hasT=tgt>0;const att=hasT?act/tgt:null;const stt=att==null?"":att>=1?"green":att>=0.7?"amber":"red";const p=period();const exp=hasT?tgt*(p.elapsed/p.working):0;const st=!hasT?"none":act>=exp?"green":act>=exp*.85?"amber":"red";const lbl=st==="green"?"On pace":st==="amber"?"At risk":"Behind pace";const noneLbl=(t.me&&isCount&&(+B.target)>0)?"Dept target":(isCount?"No client target":"No revenue target");let vs;if(hasT){vs=`<span class="mini-track"><div style="width:${clamp(att*100,0,100)}%;background:${scol(stt)}"></div></span> <b class="num" style="font-size:11.5px;color:${scol(stt)}">${pct(att,0)}</b><div style="font-size:10px;color:var(--muted)">of ${isCount?nf.format(tgt)+" clients":kMoney(tgt)}</div>`;}else if(t.me&&isCount&&(+B.target)>0){vs=`<span style="color:var(--muted);font-size:11px">combined ${nf.format(B.target)} clients<div style="font-size:10px">dept target (above)</div></span>`;}else{vs='<span style="color:var(--muted)">no target set</span>';}return `<tr class="${t.me?"me":""}"><td><div class="prow"><span class="a"${t.me?"":` style="background:${avatarCols[i%avatarCols.length]}"`}>${ini}</span><div><b>${esc(t.name)}${t.me?" · you":""}</b><span>${esc(t.title)}</span></div></div></td><td class="num">${kMoney(t.actual||0)}</td><td class="num">${nf.format(t.clients||0)}</td><td>${vs}</td><td>${st==="none"?`<span class="chip slate">${noneLbl}</span>`:`<span class="sbadge s${st[0]}"><span class="dot"></span>${lbl}</span>`}</td>${B.canNote?`<td>${t.id?`<button type="button" class="notebtn tbtn" data-bde="${t.id}" data-name="${esc(t.name)}" style="padding:5px 11px;font-size:11.5px;white-space:nowrap">✎ ${(B.notes&&B.notes[t.id]&&((((B.notes[t.id].t||"").trim())||((B.notes[t.id].c||"").trim())))?"Edit":"Add note")}</button>`:""}</td>`:""}</tr>`;}).join("")}</tbody></table></div></div>`;
      }
