...
<?php endif; ?>
      ...
      function teamTable(){
        ...
        return `<div class="card tight"><div class="table-wrap"><table><thead><tr><th>Employee</th><th>Target</th><th>${isCount?"Paid clients":"Cleared"}</th><th>Achievement</th><th>Open pipeline</th><th>Status</th>${B.canNote?'<th style="width:96px">Note</th>':""}</tr></thead><tbody>${B.team.map((t,i)=>{const tgt=isCount?(+t.clientsTarget||0):(+t.target||0);const act=isCount?(+t.clients||0):(+t.actual||0);const hasT=tgt>0;const a=hasT?act/tgt:0;const p=period();const exp=tgt*(p.elapsed/p.working);const st=!hasT?"none":act>=exp?"green":act>=exp*.85?"amber":"red";const noTlbl=isCount?"No client target":"No revenue target";const lbl=!hasT?noTlbl:st==="green"?"On pace":st==="amber"?"At risk":"Behind pace";const ini=t.name.split(/\s+/).map(x=>x[0]).slice(0,2).join("");return `<tr class="${t.me?"me":""}"><td><div class="prow"><span class="a"${t.me?"":` style="background:${avatarCols[i%avatarCols.length]}"`}>${ini}</span><div><b>${esc(t.name)}${t.me?" · you":""}</b><span>${esc(t.title)}</span></div></div></td><td class="num">${hasT?mfmt(tgt):'<span style="color:var(--faint)">—</span>'}</td><td class="num">${mfmt(act)}</td><td>${hasT?`<span class="mini-track"><div style="width:${clamp(a*100,0,100)}%;background:${scol(st)}"></div></span> <b class="num" style="font-size:11.5px">${pct(a,0)}</b>`:'<span style="color:var(--faint)">—</span>'}</td><td class="num">${t.pipeline>0?`${kMoney(t.pipeline)}<div style="font-size:9.5px;color:var(--muted);font-weight:600;margin-top:2px">expected · uncollected</div>`:`<span style="color:var(--faint)">—</span>`}</td><td>${st==="none"?`<span class="chip slate">${noTlbl}</span>`:`<span class="sbadge s${st[0]}"><span class="dot"></span>${lbl}</span>`}</td>${B.canNote?`<td>${t.id?`<button type="button" class="notebtn tbtn" data-bde="${t.id}" data-name="${esc(t.name)}" style="padding:5px 11px;font-size:11.5px;white-space:nowrap">✎ ${(B.notes&&B.notes[t.id]&&((((B.notes[t.id].t||"").trim())||((B.notes[t.id].c||"").trim())))?"Edit":"Add note")}</button>`:""}</td>`:""}</tr>`;}).join("")}</tbody></table></div></div>`;
      }
