  ...
  <div id="content">
    ...
    (() => {
      ...
      function vFinance(){
        ...
        const collCard=`<div class="card" style="border-left:4px solid var(--jade)"><div class="chead"><h4>Collection — collected vs expected</h4><span class="chip ${collShow>=80?'jade':collShow>=50?'amber':'coral'}">${collShow}% collected</span></div><div class="mini3"><div class="cm"><span>Collected</span><b class="num" style="color:var(--jade)">${fmoney(r.coll)}</b></div><div class="cm"><span>Expected (est.)</span><b class="num">${fmoney(r.exp)}</b></div><div class="cm"><span>Uncollected</span><b class="num" style="color:${uncoll>0?'var(--amber)':'var(--ink)'}">${fmoney(uncoll)}</b></div></div><div style="margin-top:16px"><div style="height:11px;border-radius:99px;background:var(--surface3);overflow:hidden;display:flex"><div style="width:${clamp(collShow,0,100)}%;background:var(--jade)"></div><div style="flex:1;background:var(--amber)"></div></div></div><div style="font-size:11px;color:var(--muted);margin-top:10px">Cleared vs list-price estimate (course price · event early-bird) · ${esc(yrLabel)}</div></div>`;
